...
public class DumpServiceImpl implements DumpService{
	...
	public void regist(String path) {
		...
		for(int i=0;i<productList.size();i++) {
			Product product = productList.get(i);//이 시점에는 아직 product_id가 미결정(insert전이기때문)
			productDAO.insert(product);//product_id가 결정
			//따라서 이 라인서부터는 product vo에 product_id가 채워져있다.
			//인서트 하자마자 이 시점부터는 product vo에 이미 pk값이 채워져 있는 상태이다..
			
			//색상넣기(하나의 상품에 딸려 있는 여러개의 색상을 넣자 그러기 위해서는 product_id가 필요)
			for(Color color:product.getColorList()) {
				color.setProduct_id(product.getProduct_id());//이 시점부터는 컬러가 fk보유했으므로 색상테이블에 데이터 넣자
			...
			//이미 들어간 파일명을 product_id+확장자 조합으로 교체 ex)35.jpg...등
