...
public class NoticeController {
	...
	public String regist(Notice notice){ //스프링에서는 vo의 멤버변수와  html의 파라미터명이 같으면
		//자동으로 데이터값이 vo에 채워진다.
		noticeService.insert(notice);//여기서 예외발생하면 실행부는 아래에 명시한 예외 핸들러 메서드를 호출
	...
	//스프링의 컨트롤러의 요청 처리 메서드 중 어느 하나라도 에러가 발생하면, 그 예외를 처리할 수 있는
	//별도의 메서드가 지원된다. 어노테이션에 명시한 예외만 감지한다.
	@ExceptionHandler(DMLException.class)
	public ModelAndView handleException(DMLException e) {
		ModelAndView mav=new ModelAndView();
		//어떤 내용을 담을지? 에러 메세지 담기
		mav.addObject("msg",e.getMessage());
		//어느 페이지를 보여줄지? message/result
		mav.setViewName("message/result");
		return mav;
	}
}
