...
public class GlobalDataAspect {
	...
	public Object getGlobalData(ProceedingJoinPoint joinPoint) throws Throwable{
		...
		String uri=request.getRequestURI();//클라이언트의 요청 URI스트링정보
		...
		if(uri.equals("/shop/member/login")||
		   ...
		   uri.equals("/admin")) {//클라이언트의 요청이 topList를 필요로 하지 않은 경우 /shop/member/login,/shop/member/regist,
			result=joinPoint.proceed();//원래 호출하려 했던 메서드 호출!!go ahead
		}else {//필요로한 경우
			//topList 가져오기
			List topList=topCategoryService.selectAll();
			Object returnObj=joinPoint.proceed();//원래 호출하려했던 메서드호출
